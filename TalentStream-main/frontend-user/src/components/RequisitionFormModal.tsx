...
import { NewRequisitionData, EmploymentType } from '../types';
...
const RequisitionFormModal: React.FC<RequisitionFormModalProps> = ({ isOpen, onClose, onAddRequisition, language = 'en' }) => {
  const isId = language === 'id';
  const [formData, setFormData] = useState<Omit<NewRequisitionData, 'headcount' | 'salaryMin' | 'salaryMax'> & { headcount: string; salaryMin: string; salaryMax: string; }>({
    title: '',
    department: '',
    location: '',
  ...
  useEffect(() => {
    if (!isOpen) {
      // Reset form
      setFormData({
        title: '', department: '', location: '', employmentType: EmploymentType.FullTime,
        headcount: '1', experienceLevel: 'Mid-level', salaryMin: '', salaryMax: '', deadline: ''
      });
      setErrors({});
    }
  }, [isOpen]);
  ...
  const validate = (): boolean => {
    ...
    if (!formData.title.trim()) newErrors.title = isId ? 'Judul posisi wajib diisi' : 'Job title is required';
    ...
    if (!formData.deadline) newErrors.deadline = isId ? 'Tenggat waktu wajib diisi' : 'Deadline is required';
    
    const headcount = Number(formData.headcount);
    if (isNaN(headcount) || headcount <= 0) newErrors.headcount = isId ? 'Harus berupa angka positif' : 'Must be a positive number';

    const salaryMin = Number(formData.salaryMin);
    const salaryMax = Number(formData.salaryMax);
    if (isNaN(salaryMin) || salaryMin < 0) newErrors.salaryMin = isId ? 'Harus angka valid' : 'Must be a valid number';
    if (isNaN(salaryMax) || salaryMax < 0) newErrors.salaryMax = isId ? 'Harus angka valid' : 'Must be a valid number';
        ...
        <form onSubmit={handleSubmit}>
          <div className="flex items-start justify-between p-5 border-b border-slate-200 dark:border-slate-700 rounded-t">
            <h3 className="text-xl font-semibold text-slate-900 dark:text-white" id="modal-title">
              {isId ? 'Buat Rekuisisi Lowongan Baru' : 'Create New Job Requisition'}
            </h3>
          ...
          <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <FormSelect label={isId ? 'Tingkat Pengalaman' : 'Experience Level'} name="experienceLevel" value={formData.experienceLevel} onChange={handleChange}>
                ...
                <FormInput label={isId ? 'Jumlah Kebutuhan (Orang)' : 'Required Headcount'} name="headcount" type="number" value={formData.headcount} onChange={handleChange} error={errors.headcount} required />
                <FormInput label={isId ? 'Tenggat Waktu Rekrutmen' : 'Hiring Deadline'} name="deadline" type="date" value={formData.deadline} onChange={handleChange} error={errors.deadline} required />
          ...
          <div className="flex items-center justify-end p-5 space-x-2 border-t border-slate-200 dark:border-slate-700 rounded-b">
            ...
            <button type="submit" className="px-4 py-2 text-sm font-semibold text-white bg-primary-600 rounded-md shadow-sm hover:bg-primary-700">
              {isId ? 'Kirim Rekuisisi' : 'Submit Requisition'}
            </button>
          </div>
        </form>
      </div>

...
export async function apiLogin(email: string, password: string): Promise<{ success: boolean; token?: string; user?: User; message?: string }> {
  try {
    ...
    if (data.user) {
      ...
      if (typeof data.user.subscription === 'string') {
        try { data.user.subscription = JSON.parse(data.user.subscription); } catch (e) {}
      }
    }
    return data;
  } catch (err) {
...
export async function apiGetCurrentUser(): Promise<{ success: boolean; user?: User; message?: string }> {
  try {
    ...
    if (u && u.id) {
      ...
      if (typeof u.subscription === 'string') {
        try { u.subscription = JSON.parse(u.subscription); } catch (e) {}
      }
